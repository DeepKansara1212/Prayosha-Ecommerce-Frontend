...
import { submitRudrakshaCalculator, getPurposes, type CalculatorResult, type SelectedLocation } from '@/api/calculators.api'
import { FormField, SelectField, LocationSearchField, DOB_PATTERN, TOB_PATTERN, MOBILE_PATTERN } from './_CalculatorShared'
...
const RudrakshaCalculatorPage: FC = () => {
  ...
  const { data: purposes } = useQuery({ queryKey: ['calculator-purposes'], queryFn: getPurposes })
  ...
  const {
    ...
    defaultValues: { name: '', dob: '', tob: '', mobile: '', purpose: '' },
  })

  const mutation = useMutation<CalculatorResult, Error, FormValues & { birthLocation: SelectedLocation }>({
    mutationFn: payload => submitRudrakshaCalculator({ ...payload, tob: payload.tob || undefined }),
    ...
    <>
      ...
      <main id="main-content">
        <div style={{ padding: 'clamp(3rem,6vw,5rem) clamp(1.25rem,5vw,4rem) clamp(2rem,4vw,3rem)' }} className="max-w-3xl mx-auto text-center">
          <SectionLabel className="justify-center">Rudraksha Calculator</SectionLabel>
          <SectionTitle as="h1" className="text-bark">
            Find your <em className="italic text-amethyst">right Rudraksha</em>
          </SectionTitle>
          <p className="font-body font-extralight text-[0.9rem] text-muted max-w-lg mx-auto mt-4 leading-relaxed">
            Tell us what you're seeking and we'll recommend Rudraksha beads suited to your purpose.
          </p>
        </div>

        <div className="max-w-xl mx-auto" style={{ padding: '0 clamp(1.25rem,5vw,4rem) clamp(3rem,6vw,5rem)' }}>
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
            <SelectField label="Purpose" error={errors.purpose?.message} {...register('purpose')}>
            ...
            <FormField label="Time of Birth (optional)" type="time" error={errors.tob?.message} {...register('tob')} />
            ...
            >
              {mutation.isPending ? 'Calculating…' : 'Find My Rudraksha'}
            </button>
          </form>
        </div>
          ...
          <div className="border-t border-warm" style={{ padding: 'clamp(2.5rem,5vw,4rem) clamp(1.25rem,5vw,4rem)' }}>
            {mutation.isPending ? (
              ...
              <EmptyState
                ...
                title="No Rudraksha mapped yet"
                description="We're still curating recommendations for this purpose — check back soon."
              />
            ) : products ? (
              <ProductGrid
                products={products}
